...
start_datenum=datenum(2018,06,17);%Irene
ndays=30;
%dirs
...
%noaa_obs_dir='/sciclone/schism10/feiye/work/NOAA_TIDE/';
...
steric_file='/sciclone/home20/whuang07/schism10/NWM/Case1/StericSL/sta150_0-700_TotalSteric_763.bp';
...
time_model=start_datenum+1/24:1/24:start_datenum+ndays;
tt=1/24:1/24:ndays;
run='RUN06a_ZG';
outname='elev.Coast_6b.RUN06a_ZG';
%load SCHISM output
output=load([prj_dir '/'  run '/PostP/' outname]);
dt = (output(2,1)-output(1,1));
elem=output(1:length(time_model),2:end);
%runo='RUN20201128'; 
...
for j=1:10
  ...
  name_fig=strcat(prj_dir,'/Figures','/SL-',run,datestr(time_model(1),'yyyymmdd'),'_',datestr(time_model(end),'yyyymmdd'),'_',num2str(j),'.png');
  saveas(gcf,name_fig)
%  savefig(name_fig)
end
...
for j=1:10
  ...
  for i=(j-1)*16+1:j*16
    ...
    %if(i<=nf&&isempty(find(id_s==i))==0)
      ...
      if(isempty(eleo{1,i})==0&&isempty(tt1{1,i})==0)
        ...
        plot(time_model,filtfilt(bf,af,double(elem(:,i))),'g');
        hold on;
  ...
  name_fig=strcat(prj_dir,'/Figures','/SLlp-',run,datestr(time_model(1),'yyyymmdd'),'_',datestr(time_model(end),'yyyymmdd'),'_',num2str(j));
