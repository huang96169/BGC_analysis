...
start_datenum=datenum(2014,01,01);%Irene
ndays=365;
%dirs
...
for i=1:nf
    ...
    if (exist(fname1,'dir')~=0)
       ...
       if(isempty(ids)==0&&isempty(ide)==0)
         [Ctmp3,IA,IC]=unique(tmp3(ids:ide));
         tmp4=C{1,2}(ids:ide);
         eleo{1,i}=interp1(Ctmp3,tmp4(IA),time_model);
  ...
  for i=(j-1)*16+1:j*16
    ...
    if(i<=nf)
      ...
      if(isempty(eleo{1,i})==0&&isempty(tt1{1,i})==0)
        ...
        title({[' VD:',cell2mat(VD{1,i})] 
              [num2str(i),': ',num2str(sa_id(i)),staname{1,i}]},'Interpreter','none','FontSize',6)
        xlim([time_model(1) time_model(end)]);
    end
  end
  name_fig=strcat(prj_dir,'/Figures','/noaaobs_',datestr(time_model(1),'yyyymmdd'),'_',datestr(time_model(end),'yyyymmdd'),'_',num2str(j),'.png');
  saveas(gcf,name_fig)
%  savefig(name_fig)
end
n
