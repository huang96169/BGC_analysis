...
start_datenum=datenum(2020,11,27);%Irene
ndays=3;
...
run='RUN20201128'; 
station_file_name='forecast143';%'upbay_sta53.moved2';%'upbay_sta53';%coast63.moved5 
...
for i=1:nf
    ...
    if (exist(fname1,'dir')~=0)
       ...
       if (isempty(C{1,1})~=1)
         ...
         %eleo{1,nfs}=interp1(datenum(tt1(ts:te)),C{1,2}(ts:te),time_model);
         eleo{1,nfs}=C{1,2};
         ss_id(nfs)=sa_id(i);
         ss_lat(nfs)=sa_lat(i);
         ss_lon(nfs)=sa_lon(i);
