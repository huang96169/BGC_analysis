...
start_datenum=datenum(2021,03,1);%Irene
ndays=30;
%dirs
...
saved_mat_dir='/sciclone/home20/whuang07/schism10/NWM/Case1/NOAATIDES/Forecast/results/'; %
...
outname='staout_1';
output=load([prj_dir '/0310_0407/'  outname]);
...
for i=1:nf
    ...
    if (exist(fname1,'dir')~=0)
       ...
       if(isempty(ids)==0&&isempty(ide)==0)
         [tmp33,ia,ic]=unique(tmp3(ids(1):ide(1)));
         tmp44=C{1,2}(ids(1):ide(1));
         eleo{1,i}=interp1(tmp33,tmp44(ia),time_model);
...
for j=1:10
  ...
  for i=(j-1)*16+1:j*16
    ...
    if(i<=nf)
      ...
      subaxis(8,2,i-(j-1)*16,'Spacing', 0.05,'MarginTop',0.05,'MarginBottom',0.02,'MarginLeft',0.03,'MarginRight',0.01);
      tmpo=eleo{1,i};
      if(isempty(eleo{1,i})==0&&isempty(tt1{1,i})==0)
        ...
        %xticks([time_model(end)-10:1:time_model(end)])
        ...
        xlim([tt(end)-10 tt(end)]);
        a = get(gca,'XTickLabel');
        set(gca,'XTickLabel',a,'fontsize',5)
      if(idxx==1)
        text(20,0,datestr(time_model(end)-10))
      end
