...
start_datenum=datenum(2018,8,24);%Irene
ndays=36;
%dirs
outname1='RUN08a_WH';
outname2='RUN08a_WH';
prj_dir1=['/sciclone/home20/whuang07/schism10/NWM/Case1/', outname1, '/PostP/'];  %one level up RUN*
prj_dir2=['/sciclone/home20/whuang07/schism10/NWM/Case1/', outname2, '/PostP/'];
...
steric_file='/sciclone/home20/whuang07/schism10/NWM/Case1/StericSL/sta150_Steric_763.txt';
figurename=[outname1,'_',outname2];
...
%   end
%end
output1=load([prj_dir1 '/staout_1']);
output2=load([prj_dir2 '/staout_1']);
elem1=output1(1:length(time_model),2:end);
elem2=output2(1:length(time_model),2:end);
dt=1/48;
...
%
%run='RUN20201128'; 
station_file_name='NGOM2';%'upbay_sta53.moved2';%'upbay_sta53';%coast63.moved5 
...
ff=fopen([prj_dir2 '/NOAAtidecompare.', outname2, '.log'],'w');
fprintf(ff,'%s\n',[outname2]);
VD=cell(1,nf);
staname=cell(1,nf);
for i=1:nf
    ...
    if (exist(fname1,'dir')~=0)
       ...
       ids=find(abs(tmp3-time_model(1))<=1/60/24);
       %ids=ids(1);
       ide=find(abs(tmp3-time_model(end))<=1/60/24);
...
for j=1:10
  ...
  for i=(j-1)*16+1:j*16
    i
    if (i==1)
          legend({'obs',outname1,outname2});
    end
    if(i>33&&i<48||i>75&&i<81)
      elem1(:,i)=elem1(:,i)+MSL_NAVD(i);
      elem2(:,i)=elem2(:,i)+MSL_NAVD(i);
    end
    if(i<=nf)
    %if(i<=nf&&isempty(find(id_s==i))==0)
      ...
      if(isempty(eleo{1,i})==0&&isempty(tt1{1,i})==0)
        id=find(isnan(tmpo)==0&isnan(elem1(:,i)')==0);
        aa=corrcoef(tmpo(id),elem2(id,i));
        cc=round(aa(1,2),2);
        %mae=mean(abs((tmpo(id))-(elem2(id,i)')));
        mae=mean(abs((tmpo(id))-(elem2(id,i)')));
        mae2(i)=round(mae,2);
        plot(time_model,detrend(tmpo),'r');hold on;
        plot(time_model,detrend((elem1(:,i))),'g');hold on
        plot(time_model,detrend((elem2(:,i))),'b');hold on
        %plot(time_model,elem2(:,i)-steric(i),'b')
        n=n+1;
        mae_navd(n)=mae2(i);
        mae_SL(n)=mean(abs((tmpo(id))-(elem2(id,i)'-steric(i))));
        %if (n==1)
        %   legend({'obs','model'});
        %end
        ...
        mae2(i)=NaN;
        cc=NaN;
        plot(time_model,((elem1(:,i))),'g');hold on;
        plot(time_model,((elem2(:,i))),'b');hold on;
        %plot(time_model,elem2(:,i)-steric(i),'b')
      end
        ...
        %xlim([time_model(end-48*10) time_model(end)]);
    end
  end
  name_fig=strcat(prj_dir2,'/Figures/', figurename, '_', datestr(time_model(1),'yyyymmdd'),'_',datestr(time_model(end),'yyyymmdd'),'_',num2str(j),'.png');
  saveas(gcf,name_fig)
%  savefig(name_fig)
end
  ...
  for i=(j-1)*16+1:j*16
    ...
    %if(i<=nf&&isempty(find(id_s==i))==0)
      ...
      if(isempty(eleo{1,i})==0&&isempty(tt1{1,i})==0)
        ...
        plot(time_model,filtfilt(bf,af,double(elem1(:,i))),'g');hold on;
        plot(time_model,filtfilt(bf,af,double(elem2(:,i))),'b');hold on;
        %plot(time_model,filtfilt(bf,af,double(elem2(:,i)))-steric(i),'b');
        n=n+1;
        %if (n==1)
        %   legend({'obs','model'});
        %end
        ...
        a = get(gca,'XTickLabel');
        set(gca,'XTickLabel',a,'fontsize',5)
        title({['CC:',num2str(cc),'ME:',num2str(mae2(i)),' VD:',cell2mat(VD{1,i})]
              [num2str(i),': ',num2str(sa_id(i)),staname{1,i}]},'Interpreter','none','FontSize',6)
        xlim([time_model(end-48*10) time_model(end)]);
    end
  end
  name_fig=strcat(prj_dir2,'/Figures/',figurename, '_lp_',datestr(time_model(1),'yyyymmdd'),'_',datestr(time_model(end),'yyyymmdd'),'_',num2str(j));
...
%ff=fopen(strcat('coast_sta','_150names.txt'),'w');
%for i=1:nf
%    fprintf(ff,'%d %s\n',[i staname{1,i}]);
%end
%fclose(ff);
