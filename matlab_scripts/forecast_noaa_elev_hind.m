...
start_datenum=datenum(2021,04,12);%Irene
ndays=10;
%dirs
...
tt=1/48:1/48:ndays;
for i=1:ndays
   outname=datestr(start_datenum+i,'yyyymmdd')
...
for j=1:10
  ...
  name_fig=strcat(prj_dir,'/Figures','/elev_hind',datestr(time_model(1),'yyyymmdd'),'_',datestr(time_model(end),'yyyymmdd'),'_',num2str(j),'.png');
  saveas(gcf,name_fig)
%  savefig(name_fig)
end
